...
export default function Loader({ size = 64, lightColor, darkColor }) {
  // Support theme-aware colors: Worker Blue vs SuperBaas Yellow
  const isWorkerTheme = typeof document !== 'undefined' && (
    document.body.classList.contains('worker-theme') ||
    (((localStorage.getItem('activeRole') || '').toLowerCase() === 'worker' || localStorage.getItem('workerAuth') === 'true') &&
      typeof window !== 'undefined' &&
      window.location.pathname !== '/' &&
      !window.location.pathname.startsWith('/worker-detail'))
  );

  const defaultLight = isWorkerTheme ? '#dbeafe' : '#fef3c7';
  const defaultDark = isWorkerTheme ? '#2563EB' : '#FDC101';

  const resolvedLight = lightColor || `var(--loader-bg, ${defaultLight})`;
  const resolvedDark = darkColor || `var(--loader-fill, ${defaultDark})`;
    ...
    >
      ...
      <svg 
        ...
        style={{
          width: '70%',
          height: '70%',
          animation: 'spin-wavy 2s cubic-bezier(0.4, 0, 0.2, 1) infinite',
          fill: resolvedDark
        }}
      >
